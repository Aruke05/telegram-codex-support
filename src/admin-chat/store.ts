...
export class AdminChatStore {
  ...
  createSession(serviceId: unknown): AdminChatSession {
    ...
    this.database.prepare(`INSERT INTO admin_chat_sessions(id,project_id,service_id,title,created_at,updated_at)
      VALUES(?,?,?,?,?,?)`).run(
      session.id, session.projectId, session.serviceId, session.title, session.createdAt, session.updatedAt,
    )
    return session
  }

  createSessionWithTurn(serviceId: unknown, question: unknown, attachments: SupportAttachmentContext[] = []): { session: AdminChatSession; turn: AdminChatTurn } {
    ...
    return this.database.transaction(() => {
      const session = this.createSession(input.serviceId)
  ...
  listSessions(serviceId?: unknown): Array<AdminChatSession & {
    ...
    const where = parsedServiceId ? "WHERE session.service_id=?" : ""
    const rows = this.database.prepare(`SELECT session.*,
      ...
      ORDER BY session.updated_at DESC,session.id DESC LIMIT 200`).all(...(parsedServiceId ? [parsedServiceId] : [])) as SqlRow[]
    return rows.map((row) => ({
      ...sessionFromRow(row),
  ...
  getSession(id: unknown): AdminChatSessionDetail {
  ...
  cancelTurn(turnId: unknown): AdminChatTurn {
  ...
  retryTurnSupersedingActive(turnId: unknown): { turn: AdminChatTurn; supersededTurnIds: string[] } {
    const parsedTurnId = idSchema.parse(turnId)
  ...
  getAttachment(id: unknown): AdminChatAttachment {
  ...
  getTurn(id: unknown): AdminChatTurn {
    return this.readTurn(idSchema.parse(id))
  }
