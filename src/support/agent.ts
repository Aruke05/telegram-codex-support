...
export class CodexSupportDecisionAgent implements SupportDecisionAgentPort {
  ...
  decide(input: SupportDecisionInput, signal?: AbortSignal): Promise<AnswerDecision> {
    const prompt = [
      ...
      "evidencePacket 是交给独立回复模型的版本化事实交接包，不发送给运营。facts 必须覆盖最终沟通可能需要的全部已取得事实，而不只覆盖工作草稿 answer；每条使用唯一 F1-F24。statement 写业务事实，evidence 写最短可复核依据，provenance 和 evidenceSource 必须与真实来源一致。代码或配置结论必须在 statement 中保留实际适用的开关、状态、分支和前置条件，不能把有条件行为概括成无条件规则。聊天转述用 certainty=reported，推断用 inferred，只有可信运行或代码证据直接确认才用 confirmed。outboundSafe=false 用于内部路径、连接信息、完整报文、密钥签名，以及对方无法独立复核或本次沟通不需要的请求体/响应体哈希、字节数、内部请求 ID、路由节点、DNS 快照等诊断元数据；只有对方明确索要或该标识确实能帮助其定位同一次请求时，才把必要关联标识标为可出站。除 decision=ignore 可为空外，requiredAnswerPoints 只列最新消息实际询问的实质要点和省略后会误导或导致错误操作的关键条件。不得把所有已查事实都列为必答项，不默认追加时间线、订单号、技术响应、对外材料、风险控制或长期方案；只有用户明确要求或本轮安全处理确实需要时才加入。仅问原因或状态时 requiredAnswerPoints 不得夹带未被询问的处理方案、当前其他配置、未来行为或后续操作，即使这些内容已在 facts 中确认；普通状态确认通常只需当前状态和必要的未知边界；详细证据留在 facts 与 investigation 供内部复核。不能只写笼统的回答用户。interaction.responseStrategy=minimal_clarification 时 communication.intent 必须是 minimal_clarification，requiredAnswerPoints 只保留最少追问以及拿到后继续核对的事项，不加入尚未确认的责任结论。unknowns 只列与本轮诉求相关且确实没有确认的具体事项，不作为必须逐项对外复述的清单；handlingNotes 写责任边界、禁止夸大和本题确有必要的核对建议，明确建议由谁核对哪个环节，不得用笼统的无法确认代替。facts 中的请求、响应和回调须在 statement 与 evidence 保留实际发送方、接收方及对应业务阶段，不能只交接一个脱离方向的成功值。communication.intent=copyable_message 时必须识别实际接收方并填写 recipient；涉及第三方沟通、责任争议、资金状态、安全或升级时 reviewLevel=strict，否则 standard。工作草稿 answer 仍按当前全部规则生成，作为新链路异常时的可用基线。",
