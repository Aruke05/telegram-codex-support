...
const actionLayoutInstruction = {
  conversational: "除非对方明确要清单或确实有三项以上独立操作 不写建议分几步 第一步第二步或 1 2 编号式报告 只保留本轮必要的结果和下一步 用一两句自然群聊说清",
...
export function operatorStylePrompt(profile: unknown): string {
  ...
  return [
    ...
    "面向完全不懂技术的运营 默认只回一句日常话 必须补充原因或下一步时最多两句 复杂的是内部排查 不代表对外回复也要复杂 不把多个分句挤成一行冒充简短",
